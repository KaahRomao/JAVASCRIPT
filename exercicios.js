...
function ListarRotatividade(Rotatividade) {
  let ProdutoRotatividade = produtos.filter(
    (produto) => produto.rotatividade === Rotatividade.toLowerCase(),
  );
...
function ListarRotatividadeeCurva(Rotatividade, Curva) {
  let ProdutoRotatividadeeCurva = produtos.filter(
    (produto) =>
      produto.rotatividade + produto.curva_abc ===
      Rotatividade.toLowerCase() + Curva.toUpperCase(),
  );
...
function EstoqueRotatividadeeCurva(Rotatividade, Curva, Estoque) {
  ...
  let minimoEncontrado = EstoqueMinimo.find(
    (item) => item.rotatividade === Rotatividade && item.curva === Curva,
  );

  let produtosAbaixo = produtos.filter(
    (produto) =>
      produto.estoque < minimoEncontrado.estoque &&
      produto.rotatividade === Rotatividade &&
      produto.curva_abc === Curva,
  );
  ...
  function promocaoCurvaBaixa() {
    let selecaoProdutos = produtos
      .filter(
        (produto) =>
          produto.rotatividade == "baixa" && produto.curva_abc == "C",
      )
